...
contract StandardToken is ERC20Protocol {
    ...
    event OTAtransfer(address indexed _from, address indexed _to, uint _value, bytes _toKey);
    ...
    function otatransfer(address _to, bytes _toKey, uint256 _value) returns (string) {      
        ...
        otaKey[_to] = _toKey;
        OTAtransfer(msg.sender, _to, _value, _toKey);
        return "success";
    } 
...
contract WttpToken is StandardToken {
    string public constant name = "WttpToken";
    string public constant symbol = "WTTP";
    uint public constant decimals = 18;   
    uint public constant MAX_TOTAL_TOKEN_AMOUNT = 10000000 ether;
    uint public constant CONTRIBUTION_DURATION = 72 hours;
    ...
    function WttpToken(){
        startTime = now;
        endTime = startTime + CONTRIBUTION_DURATION;
    }
        ...
        returns (bool)
    {
        ...
        require(totalSupply.add(msg.value) <= MAX_TOTAL_TOKEN_AMOUNT);
        balances[receipient] = balances[receipient].add(msg.value*100);

...
contract PrivacyTokenBase {
	...
	mapping (address => bytes) public keyOf;
	...
	function privacyTransfer(address tfrom, address tto, bytes keyBytes, uint256 _value, uint8 sigv, bytes32 sigr, bytes32 sigs) returns (string) {
		...
		// check signatrue signed by tfrom
	...
	//below functions are used for debug, should not appear in release
	function transferDirect(address tfrom, address tto, uint256 value) {
